...
var UI = class {
  ...
  div(...args2) {
    ...
    if (this.wrap && this.shouldApplyLayoutDSL(...args2) && typeof args2[0] === "string") {
      return this.applyLayoutDSL(args2[0]);
    }
...
var yargsParser = function Parser(args2, opts) {
...
var Y18N = class {
  ...
  __(...args2) {
    ...
    return shim.format.apply(shim.format, [this.cache[this.locale][str] || str].concat(args2));
  }
  __n() {
    const args2 = Array.prototype.slice.call(arguments);
    const singular = args2.shift();
    const plural = args2.shift();
    const quantity = args2.shift();
    let cb = function() {
    };
    if (typeof args2[args2.length - 1] === "function")
      cb = args2.pop();
    ...
    return shim.format.apply(shim.format, values.concat(args2));
  }
  setLocale(locale) {
    this.locale = locale;
  }
  ...
  _taggedLiteral(parts, ...args2) {
    ...
    return this.__.apply(this, [str].concat([].slice.call(args2, 1)));
  }
  _enqueueWrite(work) {
...
function argsert(arg1, arg2, arg3) {
  ...
  try {
    ...
    while (args2.length && args2[args2.length - 1] === void 0)
      args2.pop();
    const length = _length || args2.length;
    if (length < parsed.demanded.length) {
      throw new YError(`Not enough arguments provided. Expected ${parsed.demanded.length} but received ${args2.length}.`);
    }
...
var Completion = class {
  ...
  defaultCompletion(args2, argv, current, done) {
    ...
    const completions = [];
    this.commandCompletions(completions, args2, current);
    this.optionCompletions(completions, args2, argv, current);
    this.choicesCompletions(completions, args2, argv, current);
    done(null, completions);
  }
  commandCompletions(completions, args2, current) {
    const parentCommands = this.yargs.getInternalMethods().getContext().commands;
    if (!current.match(/^-/) && parentCommands[parentCommands.length - 1] !== current && !this.previousArgHasChoices(args2)) {
  ...
  optionCompletions(completions, args2, argv, current) {
    if ((current.match(/^-/) || current === "" && completions.length === 0) && !this.previousArgHasChoices(args2)) {
      const options = this.yargs.getOptions();
      const positionalKeys = this.yargs.getGroups()[this.usage.getPositionalGroupName()] || [];
      Object.keys(options.key).forEach((key) => {
        const negable = !!options.configuration["boolean-negation"] && options.boolean.includes(key);
        const isPositionalKey = positionalKeys.includes(key);
        if (!isPositionalKey && !this.argsContainKey(args2, argv, key, negable)) {
  ...
  choicesCompletions(completions, args2, argv, current) {
    if (this.previousArgHasChoices(args2)) {
      const choices = this.getPreviousArgChoices(args2);
      if (choices && choices.length > 0) {
        completions.push(...choices);
      }
    }
  }
  getPreviousArgChoices(args2) {
    if (args2.length < 1)
      return;
    let previousArg = args2[args2.length - 1];
    let filter = "";
    if (!previousArg.startsWith("--") && args2.length > 1) {
      filter = previousArg;
      previousArg = args2[args2.length - 2];
    }
  ...
  previousArgHasChoices(args2) {
    const choices = this.getPreviousArgChoices(args2);
    return choices !== void 0 && choices.length > 0;
  }
  argsContainKey(args2, argv, key, negable) {
    if (args2.indexOf(`--${key}`) !== -1)
      return true;
    if (negable && args2.indexOf(`--no-${key}`) !== -1)
      return true;
    if (this.aliases) {
  ...
  customCompletion(args2, argv, current, done) {
    ...
    if (isSyncCompletionFunction(this.customCompletionFunction)) {
      ...
      return this.customCompletionFunction(current, argv, (onCompleted = done) => this.defaultCompletion(args2, argv, current, onCompleted), (completions) => {
        done(null, completions);
      });
    } else {
  ...
  getCompletion(args2, done) {
    const current = args2.length ? args2[args2.length - 1] : "";
    const argv = this.yargs.parse(args2, true);
    const completionFunction = this.customCompletionFunction ? (argv2) => this.customCompletion(args2, argv2, current, done) : (argv2) => this.defaultCompletion(args2, argv2, current, done);
...
var YargsInstance = class {
  ...
  async getCompletion(args2, done) {
    ...
    if (!done) {
      return new Promise((resolve5, reject) => {
        __classPrivateFieldGet(this, _YargsInstance_completion, "f").getCompletion(args2, (err, completions) => {
          if (err)
            reject(err);
          else
      ...
      return __classPrivateFieldGet(this, _YargsInstance_completion, "f").getCompletion(args2, done);
    }
  }
  getDemandedOptions() {
    argsert([], 0);
  ...
  parse(args2, shortCircuit, _parseFn) {
    argsert("[string|array] [function|boolean|object] [function]", [args2, shortCircuit, _parseFn], arguments.length);
    this[kFreeze]();
    if (typeof args2 === "undefined") {
      args2 = __classPrivateFieldGet(this, _YargsInstance_processArgs, "f");
    }
    ...
    if (!shortCircuit)
      __classPrivateFieldSet(this, _YargsInstance_processArgs, args2, "f");
    if (__classPrivateFieldGet(this, _YargsInstance_parseFn, "f"))
      __classPrivateFieldSet(this, _YargsInstance_exitProcess, false, "f");
    const parsed = this[kRunYargsParserAndExecuteCommands](args2, !!shortCircuit);
  ...
  parseAsync(args2, shortCircuit, _parseFn) {
    const maybePromise = this.parse(args2, shortCircuit, _parseFn);
    return !isPromise(maybePromise) ? Promise.resolve(maybePromise) : maybePromise;
  }
  parseSync(args2, shortCircuit, _parseFn) {
    const maybePromise = this.parse(args2, shortCircuit, _parseFn);
  ...
  [kCreateLogger]() {
    return {
      log: (...args2) => {
        ...
        __classPrivateFieldSet(this, _YargsInstance_output, __classPrivateFieldGet(this, _YargsInstance_output, "f") + args2.join(" "), "f");
      },
      error: (...args2) => {
        if (!this[kHasParseCallback]())
          console.error(...args2);
  ...
  [kParsePositionalNumbers](argv) {
    ...
    for (let i = 0, arg; (arg = args2[i]) !== void 0; i++) {
  ...
  [kRunYargsParserAndExecuteCommands](args2, shortCircuit, calledFromCommand, commandIndex = 0, helpOnly = false) {
    let skipValidation = !!calledFromCommand || helpOnly;
    args2 = args2 || __classPrivateFieldGet(this, _YargsInstance_processArgs, "f");
    ...
    const parsed = __classPrivateFieldGet(this, _YargsInstance_shim, "f").Parser.detailed(args2, Object.assign({}, __classPrivateFieldGet(this, _YargsInstance_options, "f"), {
    ...
    try {
      ...
      if (requestCompletions) {
        if (__classPrivateFieldGet(this, _YargsInstance_exitProcess, "f"))
          setBlocking(true);
        args2 = [].concat(args2);
        const completionArgs = args2.slice(args2.indexOf(`--${__classPrivateFieldGet(this, _YargsInstance_completion, "f").completionKey}`) + 1);
...
// src/cli.js
var args = yargs_default(process.argv.slice(2)).config("config", function(configPath) {
